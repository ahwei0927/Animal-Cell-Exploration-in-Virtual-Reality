...
namespace BNG {
    ...
    public class VREmulator : MonoBehaviour {
        ...
        void Start() {
            ...
            if(gameObject.transform.GetChild(0).transform.Find("CameraRig")) {
                mainCameraTransform = gameObject.transform.GetChild(0).transform.Find("CameraRig").transform;
            }
            // Oculus Rig Setup
            else if(gameObject.transform.GetChild(0).Find("OVRCameraRig")) {
                mainCameraTransform = gameObject.transform.GetChild(0).transform.Find("OVRCameraRig").transform;
            }

            if (leftHandAnchor == null)
            { leftHandAnchor = gameObject.transform.GetChild(0).GetChild(0).GetChild(0).Find("LeftHandAnchor").transform; }
            if (rightHandAnchor == null)
            { rightHandAnchor = gameObject.transform.GetChild(0).GetChild(0).GetChild(0).Find("RightHandAnchor").transform; }

            leftControllerTranform = leftHandAnchor.Find("LeftControllerAnchor").transform;
            rightControllerTranform = rightHandAnchor.Find("RightControllerAnchor").transform;
